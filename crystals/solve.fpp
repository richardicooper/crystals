...
C--SET UP THE PARAMETER GROUP SHIFT LOCATIONS
      ...
      STORE(I+4)=0.
      STORE(I+5) = 0.
C--CHECK IF A SHIFT FACTOR HAS BEEN GIVEN IN LIST 23
      IF(ISTORE(JA))1050,1100,1100
C--MOVE THE SHIFT FACTOR FROM LIST 23
1050  CONTINUE
      STORE(I)=STORE(JB)
...
1300  CONTINUE
      CALL XPCM(1)
C--SET A FEW LIST 11 CONSTANTS
1350  CONTINUE
      D=STORE(L11P+17)
      IF ( ABS( STORE(L11P + 16)) .LE. ZERO) THEN
            WRITE( NCAWU,1351) D,STORE(L11P+16)
            IF (ISSPRT .EQ. 0) WRITE( NCWU,1351) D,STORE(L11P+16)
            WRITE ( CMON, 1351)  D,STORE(L11P+16)
            CALL XPRVDU(NCVDU, 3,0)
1351      FORMAT(
     1 ' Insufficient data to refine all variables',/
     2 '          No of observations = ',I10,/
     3 '             No of variables = ',I10,/)
            E = 0.
      ELSE
      E=D/STORE(L11P+16)
C--CHECK THAT THE NUMBER OF DEGREES OF FREEDOM IS POSITIVE
      IF(E)1400,1500,1500
C--NUMBER OF DEGREES IF FREEDOM IS NEGATIVE
1400  CONTINUE
      CALL XERHDR(0)
...
4550  CONTINUE
C--CONVERT THE POSN. COORDS. TO ANGSTROM
      JF=JC+5*MW
      JG=L1P1
      DO 4600 I=1,3
...
C--PRINT THE RESULTS
      IF (ISSPRT .EQ. 0) THEN
C      WRITE(NCWU,4650)STORE(JC+2),(STORE(I+2),I=JE,JD,MW),(A1(I),I=1,3)
C     2 ,STORE(JC+3),(STORE(I+3),I=JE,JD,MW),(A1(I),I=4,6)
      WRITE(NCWU,4650)STORE(JC+2),(STORE(I+2),I=JE,JC+13*MW,MW),
     2 (A1(I),I=1,3),
     3 STORE(JC+3),(STORE(I+3),I=JE,JC+13*MW,MW),(A1(I),I=4,6)
4650  FORMAT(/13X,'Overall',4X,'Occ',5X,'U[iso]',4X,'X',8X,'Y',8X,'Z',
     2 7X,'U[11]',4X,'U[22]',4X,'U[33]',4X,'U[23]',4X,'U[13]',4X,
     3 'U[12]'//' Mean      ',12F9.5/38X,3F9.5//' R.M.S.    ',12F9.5/
     4 38X,3F9.5,A1//' Multiplier',12F9.5///' Shifts for X,',
     5 ' Y and Z are expressed in fractions and angstrom.'/)
C      WRITE(NCWU,4660) STORE(JC+5),(STORE(I+5),I=JE,JD,MW),
C     1 STORE(JC),(STORE(I),I=JE,JD,MW)
      WRITE(NCWU,4660) STORE(JC+5),(STORE(I+5),I=JE,JC+13*MW,MW),
     1 STORE(JC),(STORE(I),I=JE,JC+13*MW,MW)
      ENDIF
...
4770  FORMAT(/13X,'UisoSph',2X,'RadSph',3X,
     ...
     4 //' Mean      ',10F9.5//' R.M.S.    ',10F9.5,A1
     5 // ' Multiplier',10F9.5/)
      WRITE(NCWU,4775) (STORE(I+5),I=JC+14*MW,JC+23*MW,MW),
     1 (STORE(I),I=JC+14*MW,JC+23*MW,MW)
      ENDIF
...
4950  CONTINUE
      IF(STORE(I+3)-ZEROSQ)5050,5050,5000
5000  CONTINUE
...
2700  CONTINUE
      ...
      WRITE(CTEXT,'(2A4,I1)') (OVP(JRR,JS),JRR=1,2)
       WRITE(NCWU,2750)FLAG,JT,JX,STORE(JO),C,A,S,(OVP(JRR,JS),JRR=1,2)
      ENDIF
      IF (ITYPE .EQ. 1 )
     1 WRITE(NCAWU,2790)FLAG,JT,JX,STORE(JO),C,A,S,(OVP(JRR,JS),JRR=1,2)
...
2850  CONTINUE
      IF (IHIT .EQ. 1) ISTORE(JO+1) = JC
      WRITE(CTEXT,'(2A4,I1)')  (SCALE(NC,NA+2),NC=1,2),JS
      IF ((IPRINT .EQ. 1 ) .AND. (ISSPRT .EQ. 0) ) THEN
       WRITE(NCWU,2750)FLAG,JT,JX,STORE(JO),C,A,S,
     2 (SCALE(NC,NA+2),NC=1,2),JS
      ENDIF
      IF (ITYPE .EQ. 1)
...
C-C-C-DISTINCTION BETWEEN ANISO'S AND ISO/SPECIAL'S FOR PRINT
     ...
     2         NINT(STORE(M5+1)),(ICOORD(JRR,JS+7),JRR=1,NWKA)
               ELSE
               WRITE(NCWU,3050)FLAG,JT,JX,STORE(JO),C,A,S,STORE(M5),
...
C-C-C-THE FOLLOWING IF-BLOCK (8 LINES) ARE COMMENDED OUT,
C-C-C-BECAUSE IT CAUSES DIFFICULTIES WITH KSET52
C-C-C-(LISTS 5 AND 12 DON'T CORRESPOND):
C            IF (STORE(IADDU+1) .LT. UMIN) THEN
CC--THIS ANISOTROPIC TEMPERATURE FACTOR IS NOT ALLOWED
CC                  STORE(M5+3) = UMIN + ZERO
CC-C-C-SET MINIMUM-VALUE FOR ISOTROPIC (!) TEMP.FACTOR
...
1400  FORMAT(///1X,I2,'  Overall parameter(s)'/)
      IF (ISSPRT .EQ. 0) THEN
      WRITE(NCWU,1450)((OVP(I,J),I=1,2),J=1,NKO)
      ENDIF
...
1950  CONTINUE
      IF (I .LE. 3) THEN
      IF (ISSPRT .EQ. 0) THEN
            WRITE(NCWU,2000)ICOM05(J+2),(SCALE(L,I+2),L=1,2)
      ENDIF
      ELSE
      IF (ISSPRT .EQ. 0) THEN
            WRITE(NCWU,2001)ICOM05(J+2),(SCALE(L,I+2),L=1,2)
      ENDIF
      ENDIF
...
C-C-C-PARAMETERS FOR ANISOTROPIC ATOM
      WRITE(NCWU,2250)((ICOORD(I,J),I=1,2),J=1,NKA-4)
...
2600  FORMAT(/5X,A4,F9.0,1X,12F9.5)
C--CHECK IF THE E.S.D.'S ARE TO BE PRINTED
      IF(IPRINT)2850,2650,2850
2650  CONTINUE
C-C-C-IS THE NEXT STATEMENT REASONABLE ? (N5A NOT USED WITH THIS VALUE)
      N5A=NKA-2
      IF (ISSPRT .EQ. 0) THEN
C      WRITE(NCWU,2700)APD(1),(APD(J-2),J=5,NKA)
C-C-C-NKA IS 18 NOW INSTEAD OF 14
      WRITE(NCWU,2700)APD(1),(APD(J-2),J=5,NKA-4)
      ENDIF
2700  FORMAT(19X,F9.5,9X,10F9.5)
      IF (ISSPRT .EQ. 0) THEN
C      WRITE(NCWU,2700)BPD(1),(BPD(J-2),J=5,NKA)
      WRITE(NCWU,2700)BPD(1),(BPD(J-2),J=5,NKA-4)
      ENDIF
