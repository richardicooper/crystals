...
2020    CONTINUE
      END IF
C--CHECK IF WE SHOULD OUTPUT THIS MAP SECTION TO TAPE
      IF(IOUTAP)2100,2100,2050
C--OUTPUT THIS SECTION TO M/T
2050  CONTINUE
      M81=L81+NYNZ-1
      WRITE(MT1)NYNZ,(FSTORE(I),I=L81,M81)
CRIC99
C&GID      IF (ISSTML .EQ. 4)
C&GID     1 CALL FOUR3D(NYNZ,FSTORE(L81),1.0)
C
...
C----- WRITE THE M/T HEADER  DETAILS
     ...
     1                        I=L14,2*MD14+L14, MD14)
      STEPS(1)=STORE(L14+1)
      STEPS(2)=STORE(L14+MD14+1)
      STEPS(3)=STORE(L14+(2*MD14)+1)
      START(1)=STORE(L14)
      START(2)=STORE(L14+MD14)
      START(3)=STORE(L14+(2*MD14))
C      TRANSF(12),CELL(6),SIZE(3),START (3),STEPS(3)
&GID      IF (ISSTML .EQ. 4)
&GID     1 CALL FOUR3DI(STORE(NFL),STORE(L1P1),NUM(1),
&GID     2 START(1),STEPS(1),.FALSE.)
3800  CONTINUE
      RETURN
C
9900  CONTINUE
