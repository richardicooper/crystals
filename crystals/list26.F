...
C
      DATA IVERSN /404/
C
&HOL      DATA DIST(1)/4Hdist/,DIST(2)/4Hance/,DIST(3)/4Hs   /
&HOL      DATA ANGLE(1)/4H ang/,ANGLE(2)/4Hles /,ANGLE(3)/4H    /
&HOL      DATA MSD(1)/' M.S'/,MSD(2)/'.D.'''/,MSD(3)/'s   '/
&HOL      DATA STROM(1)/4HAngs/,STROM(2)/4Htrom/
&HOL      DATA DEG(1)/4Hdegr/,DEG(2)/4Hees /
&HOL      DATA UIJ(1)/4H  U(/,UIJ(2)/4HIJ)'/,UIJ(3)/4Hs   /
&HOL      DATA CONT(1)/4Hcont/,CONT(2)/4Hacts/,CONT(3)/4H    /
C
#HOL      DATA DIST(1)/'dist'/,DIST(2)/'ance'/,DIST(3)/'s   '/
#HOL      DATA ANGLE(1)/' ang'/,ANGLE(2)/'les '/,ANGLE(3)/'    '/
#HOL      DATA MSD(1)/' M.S'/,MSD(2)/'.D.'''/,MSD(3)/'s   '/
#HOL      DATA STROM(1)/'Angs'/,STROM(2)/'trom'/
#HOL      DATA DEG(1)/'degr'/,DEG(2)/'ees '/
#HOL      DATA UIJ(1)/'  U('/,UIJ(2)/'IJ)'''/,UIJ(3)/'s   '/
#HOL      DATA CONT(1)/'cont'/,CONT(2)/'acts'/,CONT(3)/'    '/
C
...
4700  CONTINUE
      IF (ISSPRT .EQ. 0) THEN
      WRITE(NCWU,4750)KF,(OVP(M,JC),M=1,2), (A1(M),M=1,MM)
      ENDIF
4750  FORMAT(2H  ,A1,41X,2A4,4X,2F10.5)
      WRITE(NCAWU,4751)KF,(OVP(M,JC),M=1,2), (A1(M),M=1,MM)
      IF ((IMON .GE. 1) .AND. (ABS(A1(2)) .GE. C) ) THEN
       WRITE(CMON,4751)KF,(OVP(M,JC),M=1,2), (A1(M),M=1,MM)
       CALL XPRVDU(NCVDU, 1,0)
      ENDIF
...
4800  CONTINUE
      JB=ISTORE(JA+1)
      IF (ISSPRT .EQ. 0) THEN
      WRITE(NCWU,4850) KF,(SCALE(M,JB),M=1,2), (A1(M),M=1,MM)
      ENDIF
4850  FORMAT(2H  ,A1,38X,2A4,I4,3X,2F10.5)
      WRITE(NCAWU,4851) KF,(SCALE(M,JB),M=1,2), (A1(M),M=1,MM)
      IF ((IMON .GE. 1) .AND. (ABS(A1(2)) .GE. C) ) THEN
       WRITE(CMON,4851) KF,(SCALE(M,JB),M=1,2), (A1(M),M=1,MM)
       CALL XPRVDU(NCVDU, 1,0)
      ENDIF
