...
C
C Revision 1.2  2005/01/17 13:28:39  rich
C Bring new cvs repository into line with old. (Make max weight a real number in L4).
C
...
1050  CONTINUE
      ...
      STORE(I+5) = STORE(JA+21)          !STORE THE QUASI MULTIPLIER
      GOTO 2300
C
...
CODE FOR XGETAT
      SUBROUTINE XGETAT(IULN,ITYPE)
C--GET ATOM REFERENCES IN LEXICAL LISTS IN FORMAT 'TYPE(SERIAL)'.
C  STUB TO LINK TO STEFAN'S SPARSE MATRIX CODE
C
C  IULN    THE LIST NUMBER TO BE PRINTED.
C  ITYPE   THE TYPE OF GET REQUIRED :
C
C           1  PUNCH A LIST OF ATOMS (ONLY OPTION YET)
C
C--
      INCLUDE 'HEADES.INC'
      INCLUDE 'ISTORE.INC'
...
1200  CONTINUE
      N=N+1
      IF ( ITYPE .EQ. 1) THEN
         CALL XPCHAT (N,IBUFF(4),IBUFF(5),IBUFF(6))
      ENDIF
      GOTO 1100
C--CRACKED CODE  -  CHECK IF WE SHOULD PRINT IT
1250  CONTINUE
...
C--PRINT THE CODE  -  SET UP THE REQUIRED POINTERS
1300  CONTINUE
c----- we dont want the cracked code
      GOTO 1100
      END

C
CODE FOR XPCHAT
      SUBROUTINE XPCHAT(NUMB,IADDD,LENGRP,NGRP)
C--GET ATOM REFERENCES IN LEXICAL LISTS IN FORMAT 'TYPE(SERIAL)'.
C      (SEE ALSO XPRTLC, XPCHLX)
C
C  NUMB    THE CARD NUMBER.
C  IADDD   THE DISC ADDRESS OF THE LOGICAL CARD.
C  LENGRP  THE LENGTH OF EACH REAL CARD.
...
C--CHECK IF THIS IS THE FIRST CARD
      ...
      DO 2000 I=1, NCHAR-1
       IX=INDEX(CLINE(I:I), '(')
       IF (IX .EQ. 0) GOTO 2000
        DO 2050 J=I+1, NCHAR 
         IF( INDEX('0123456789', CLINE(J:J)) .NE. 0) GOTO 2050
         IF( INDEX(')', CLINE(J:J)) .NE. 0) GOTO 2000
         CLINE(J:J) = ' '
2050    CONTINUE
2000  CONTINUE
      CALL XCREMS(CLINE, CLINE, NCHAR)
      IX = INDEX(CLINE, '( )')
      IF (IX .NE. 0) CLINE(IX:IX+2) = '  '
      IX = INDEX(CLINE, ' )')
      IF (IX .NE. 0) CLINE(IX:IX+1) = ') '
      CALL XCREMS(CLINE, CLINE, NCHAR)
      CALL XCTRIM (CLINE, NCHAR)
      KK = MIN0 ( 80, NCHAR)
