...
C
CODE FOR KATOMT
      FUNCTION KATOMT(LOCD,LOCA,LOCB)
C--APPLY THE INVERSE TRANFORMATION TO AN ATOM
C
C  LOCD  ADDRESS OF THE ATOM BLOCK HEADER , WHOSE DIRECTIVES ARE
C        TO BE APPLIED IN INVERSE
C  LOCA  LOCATION OF THE ATOMIC COORDINATES TO BE TRANFORMED
C  LOCB  OUTPUT ADDRESS
C
C--RETURN VALUES ARE :
C
C  -1  ERROR IN FORMING THE ATOM
...
C--SET THE INITIAL ADDRESSES
      ...
      M5=M5+1
      L=(ISTORE(LOCD+8)-1)*3+L2P
      M=(IABS(ISTORE(LOCD+7))-1)*MD2I+L2I+9
      N=LOCD+9
C--MOVE THE SERIAL, OCC AND U[ISO]
C-C-C-MOVE THE SERIAL, OCC AND FLAG
C--REMOVE THE TRANSLATIONAL ELEMENTS
C-C-C-AND TRANSFER X,Y,Z TO PROVISIONAL PLACE (U11,U22,U33)
      DO 1000 I=J,K
      STORE(M5)=STORE(I)
      STORE(M5+6)=STORE(I+3)-FLOAT(ISTORE(N))-STORE(L)+STORE(M)
      M5=M5+1
      N=N+1
      M=M+1
      L=L+1
1000  CONTINUE
      M=M-12
C--APPLY THE INVERSE SYMMETRY OPERATION
      CALL XMLTTM(STORE(M),STORE(M5+1),STORE(M5),3,3,3)
C--CHECK IF A CENTRE OF SYMMETRY HAS BEEN USED
      IF(ISTORE(LOCD+7))1050,1150,1150
C--REMOVE THE CENTRE OF SYMMETRY
1050  CONTINUE
      J=M5
      K=M5+2
      DO 1100 I=J,K
...
1150  CONTINUE
C-C-C-CHECK WHETHER ATOM IS ANISOTROPIC OR ISOTROPIC/SPHERE/LINE/RING
      IF(ABS(STORE(LOCB+3))-UISO)1200,1250,1250
C-C-C-TRANSFORM THE ANISOTROPIC TEMPERATURE FACTORS
      IF(ABS(STORE(LOCB+3))-UISO)1200,1250,1250
1200  CONTINUE
      J=M5A+7
...
C-C-C-ATOM IS ISOTROPIC/SPHERE/LINE/RING
1250  CONTINUE
C-C-C-CHECK WHETHER ATOM IS ISOTROPIC, SPHERE OR LINE/RING
      IF(NINT(ABS(STORE(LOCB+3))) .EQ. 1) THEN
C-C-C-ATOM IS ISOTROPIC
C-C-C-TRANSF. OF ISOTR. TEMP.FACT.
      STORE(LOCB+7)=STORE(LOCA+7)
C      J=LOCB+7
