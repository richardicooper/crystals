...
C             -2 USE THE FUNCTION VECTOR FOR EXPLICIT PIVOTS.
C             -3 USE THE FUNCTION VECTOR FOR EXPLICIT BONDED ATOMS.
...
240   CONTINUE
      JFNVC = -1
      GOTO 220
C----- PIVOT. INDICATE FUNCTION VECTOR ONLY USED FOR EXPLICIT PIVOTS
250   CONTINUE
      JFNVC = -2
      GOTO 220
C----- BONDED. INDICATE FUNCTION VECTOR ONLY USED FOR EXPLICIT BONDED ATOMS
260   CONTINUE
      JFNVC = -3
      GOTO 220
...
C----- RESET FUNCTION VECTOR AND LIMITS IF NECESSARY
      IF((JFNVC.EQ.1).OR.(JFNVC.LE.-2)) THEN
        BMAX = 2.*BMAX + TOLER
        AT = CHECK
        AC = BMAX
        BT = AT
...
C
      IF ((JFNVC .EQ. 0) .OR. (JFNVC .EQ. -1)) THEN
...
C -- IGNORE function vector, if searching for BONDED atoms.
      IF ( ( ISTORE(MFNVCA) .LE. 0 ) .AND. ( JFNVC .NE. -3 ) ) GOTO 5900
      M12=L12
      M5=L5
      MFNVC = LFNVC
...
2185  FORMAT(1X,A4,1X,F4.0,1X,I3,50(1X,A4,1X,F4.0))
         WRITE(NCPU,2185) STORE(M5P), STORE(M5P+1), (JS-NFLBAS)/NW,
     1   (STORE(ISTORE(J)),STORE(ISTORE(J)+1),J=NFLBAS,JS,NW)
      ENDIF
...
C JFNVC SET TO -2 IF VCTOR USED FOR EXPLICIT PIVOTS
C JFNVC SET TO -3 IF VCTOR USED FOR EXPLICIT BONDED ATOMS
...
1200  CONTINUE
C----- CHECK IF THIS ATOM IS EXCLUDED
C RIC AUG 00 > I think this 
C      IF ((JFNVC .EQ. -1) .AND. (ISTORE(MFNVC) .EQ. -1)) GOTO 2725
C is supposed to be this:
C RIC JAN 01 The EXCLUDE flags are ignored by ONLY, BONDED & PIVOT.
...
C1749  CONTINUE
C      WRITE ( CMON ,'(A,I4)') 'Reject after ', NH
