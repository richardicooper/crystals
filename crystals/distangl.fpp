...
5660  CONTINUE
      IF (ISSPRT .EQ. 0) THEN
      WRITE(NCWU,5700)AKEY,(IAPD(NA),NA=1,NC)
      ENDIF
...
5760  CONTINUE
      IF (ISSPRT .EQ. 0) THEN
      WRITE(NCWU,5800)AKEY,(IAPD(NA),NA=1,NC)
      ENDIF
...
2150  CONTINUE
      IF(KCOMP(1,PEAK(1),STORE(M5A),1,MD5A))2200,2200,2450
...
C      FOURIER REFINEMENT
      IF (ITYPE .NE. 1) THEN
       IF(KCOMP(1,PEAK(1),STORE(K),1,MD5A))2400,2400,2250
      ENDIF
...
C---- REPLACE ATOM HEIGHT BY CORRESPONDING PEAK HEIGHT
      IF( (KCOMP(1,PEAK(1),STORE(KKKI),1,MD5A) .GE. 1) .AND.
...
C----- ONLY DELETE PEAKS.
        K = ISTORE(I)
        IF ( KCOMP(1, PEAK(1), STORE(K), 1, MD5A) .LE. 0) GOTO 2900
      ENDIF
