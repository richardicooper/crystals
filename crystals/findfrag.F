...
C
C Revision 1.12  2003/09/03 20:57:51  rich
C Change some more common blocks into macros.
C
C Revision 1.11  2003/08/05 11:11:11  rich
C Commented out unused routines - saves 50Kb off the executable.
C
C Revision 1.10  2003/06/09 11:30:47  rich
C Change another explicit common block to macro in findfrag.
C
C Revision 1.9  2002/12/16 18:23:15  rich
C Change to KDIST1 call.
C
C Revision 1.8  2002/10/02 13:39:51  rich
...
C
C Revision 1.5  2001/02/26 10:25:32  richard
...
C BITWISE NOT OF THE INTEGER N
      ...
      END
cCODE FOR KSPEAT
c      FUNCTION KSPEAT (MSTRUC, ISEQ)
cC
cC GIVES THE ATOMIC SPECIES OF THE ISEQ'TH ATOM OF THE STRUCTURE AT
cC MSTRUC AS AN INTEGER.  PUT SPECIFIC ATOM INFO TREATMENT IN HERE.
c
c      CHARACTER*3 CATFLA, CSERFL, CSPEFL
c      CHARACTER*1 CPLUS, CMINUS, CTIMES, CDIV
c      CHARACTER*10 CDIGIT
...
c\QSTORE
cC      INCLUDE 'FIND.INC'
c      IATYPE = ISTORE (MSTRUC+10)
c      IF (ISEQ .GT. ISTORE(MSTRUC+12)) THEN
cC            NERRFI = NERRFI_WRONG_ATOM_SEQUENCE
c            NERRFI = 29
c            GO TO 9900
c      END IF
c      IADDAT = ISTORE(MSTRUC+13) + (ISEQ-1)*ISTORE (MSTRUC+14)
c      IF ((IATYPE.EQ.1) .OR. (IATYPE.EQ.2) .OR. (IATYPE.EQ.3)) THEN
c            KSPEAT = ISTORE (IADDAT)
c      ELSE
cC            NERRFI = NERRFI_BAD_ATTYPE
c            NERRFI = 30
c      END IF
c9900  CONTINUE
c      RETURN
...
9800  CONTINUE
      ...
      END
cCODE FOR OUTSET
c      SUBROUTINE OUTSET (CLOCAT, IMODE, LPSLW)
cC
cC OUTPUT ALL THE SETS ETC
cC IMODE=
cC 1   DISPLAY ALL THE SETS
cC 2   DISPLAY JUST THE PS
cC 3   DISPLAY JUST THE SET AT LPSLW
cC
c\XIOBUF
c\XUNITS
c      LOGICAL LPRESE
...
c        CINFO = 'CURRENT '
c      ELSE
c        WRITE (CMON,'(A)') 'INCORRECT IMODE VALUE IN CALL TO OUTSET'
...
c      WRITE (CMON,'(A)') '-------------------------------------------'
c      CALL XPRVDU(NCVDU, 1,0)
c      WRITE (CMON, '(1X, A8, A12,A)') CINFO, ' INFO FROM: ', CLOCAT
c      CALL XPRVDU(NCVDU, 1,0)
c      WRITE (CMON,'(1X,A33)')' LSETS  NSETS  LNEW  LPSL  NFL '
c      CALL XPRVDU(NCVDU, 1,0)
c      WRITE (CMON,'(5(1X,I6))') LSETS, NSETS, LNEW, LPSL, NFL
c      CALL XPRVDU(NCVDU, 1,0)
c
c      DO 1000, MSETS= LUSE, LUSE+(NUSE-1)*MDSETS, MDSETS
cC
cC LOOP OVER ALL THE STRUCTURES
c        MOFF = 1
c        DO 500, MSTR = LSTRUC, LSTRUC + (NSTRUC-1)*MDSTRU, MDSTRU
c          WRITE (CMON, '(''    ('', I2,'')'',$)')
c     1               1+((MSTR - LSTRUC)/MDSTRU)
c          CALL XPRVDU(NCVDU, 1,0)
c          WRITE (CMON, '('' ['',I4,'']'')') ISTORE (MSETS +MOFF)
c          CALL XPRVDU(NCVDU, 1,0)
c          NNO = ISTORE (MSTR)
c          NSE = ISTORE (MSTR+15)
c          WRITE (CMON, '(A1)') '{'
c          CALL XPRVDU(NCVDU, 1,0)
c            DO 498, MNO = 1, NNO
c            IF (LPRESE (MNO, MSETS, MOFF)) THEN
c              WRITE (CMON, '(1X,I4)')  MNO
c              CALL XPRVDU(NCVDU, 1,0)
c            END IF
c498       CONTINUE
c          WRITE (CMON,'(A1)') '}'
...
C
      ...
      END
cCODE FOR XINSTR
c      SUBROUTINE XINSTR ( NQUERY )
cC
cC INITIALIZE (AND ZEROISE) A LIST CONTAINING GENERAL INFO ABOUT THE
cC STRUCTURES BEING CONSIDERED
cC
cC THE CURRENT FORMAT OF AN ENTRY IN THE GENERAL LIST IS:
cC
cC > NO OF NODES IN THE GRAPH CORRESPONDING TO THIS STRUCTURE
cC > NO OF NODES OF THIS STRUCTURE AS YET UNASSIGNED (USED IF LOVERL
cC   FALSE OR LMULTI TRUE)
cC > ADDRESS OF THE FIRST ENTRY IN THE FIXED PART OF THE NODE LIST
cC   DESCRIBING THE GRAPH
cC > ADDRESS OF THE FIRST ENTRY OF THE AREA AT THE END OF THE GENERAL
cC   STRUCTURE LIST HOLDING DETAILS OF THE ISOMORPHISMS (DESCRIBED IN
cC   XCRLIF)
cC > NO OF ISOMORPHISMS FOUND WITHIN THE SUPERSTRUCTURE FOR THIS SUBSTR
cC > TYPE OF ACTION TO BE TAKEN ON FINDING THE ISOMORPHISM
cC   1 = SIMPLY WRITE OUT DETAILS TO THE SCREEN
cC > DETAILS OF WHAT TO DO WITH THE ISOMORPHISM (EG COMMANDS STORE)
cC > DITTO (DESCRIBED IN       )
cC > DITTO (DESCRIBED IN       )
cC > TYPE OF ACTUAL STRUCTURE INFO (EG LIST 5 )
cC   1 = LIST 5 ACTUAL
cC   2 = LIST 10 ACTUAL
cC   3 = SAME FORMAT AS LIST 5, BUT START AND NO OF ENTRIES MUST BE
cC       SPECIFIED (MD5 STAYS THE SAME)
cC   4 = SAME FORMAT AS LIST 10, BUT START AND NO OF ENTRIES MUST BE
cC       SPECIFIED
cC   5 = GRAPH LIST ONLY (FROM DATA FILE, FORMAT DESCRIBED IN XGRGEN)
cC > DETAILS OF ACTUAL STRUCTURE INFO
cC > DITTO (DESCRIBED IN XGRGEN)
cC > DITTO (DESCRIBED IN XGRGEN)
cC
cC GLOBALS:
cC LSTRUC = START OF STRUCTURES LIST
cC NSTRUC = NUMBER OF STRUCTURES BEING CONSIDERED
cC MDSTRU = LENGTH OF AN ENTRY
cC
cC LOCALS:
cC NQUERY = NO OF QUERY STRUCTURES
cC NROOM  = AMOUNT OF ROOM REQUIRED BY GEN STR LIST
cC
c
c      CHARACTER*3 CATFLA, CSERFL, CSPEFL
c      CHARACTER*1 CPLUS, CMINUS, CTIMES, CDIV
...
c      LSTRUC = NFL
cC
cC MAKE ROOM FOR THE GENERAL STRUCTURE LIST
c      NSTRUC = NQUERY + 1
c      NROOM  = NSTRUC * MDSTRU
c      NFL = KCHNFL (NROOM)
cC
c      DO 1000, M = 1, NROOM
c           ISTORE(LSTRUC+M-1) = 0
c1000  CONTINUE
c      RETURN
c      END
CODE FOR XINTER
...
C --  COMPUTE DISTANCE STACK
      NDISTA = KDIST1( IN, JS, JT, 0, TOLER, ITRANS, 0, 4, 0)
      NFL    = JS
C
      IF (NDISTA .GT. 0) THEN
C
...
C      WRITE (6, *) ' '
      ...
      END
cCODE FOR XNPA
c      SUBROUTINE XNPA (LPSTT, ISTA, IQTA)
cC
cC WORKS OUT THE NEXT POSSIBLE ASSIGNMENT TO MAKE ON A PS.
cC
cC LPSTT = BEGINNING OF THE PS
cC ISTA  RETURNED AS THE NODE OF THE STRUCTURE SET THAT CAN BE ASSIGNED
cC IQTA  RETURNED AS THE QUERY NODE TO BE ASSIGNED
c
c      CHARACTER*3 CATFLA, CSERFL, CSPEFL
c      CHARACTER*1 CPLUS, CMINUS, CTIMES, CDIV
c      CHARACTER*10 CDIGIT
...
cC
cC LOOP OVER THE PS
c      DO 3000, M = LPSTT, LPSTT+(NNODEQ*MDSETS), MDSETS
cC
cC CHECK FOR STRUCTURE SETS OF CARDINALITY > 1.  USE THE FIRST ONE OF
cC THESE TO MAKE AN ASSIGNMENT.
c            ICARDS = ISTORE(M+1)
c            IF      (ICARDS .GT. 1) THEN
c                 IQTA   =  ((M - LPSTT) /MDSETS) + 1
cC
cC NOW LOCATE THE STRUCTURE NODE
c                 DO 2000, MSTR = 1, NINTSU-1
c                       NTMP  = ISTORE (M+MSTR+1)
c                       IF (NTMP .GT. 0) THEN
c                             DO 1000, MBITS=1, L1INT
c                                   NTMP2 = N2POWS(MBITS-1)
c                                   IF (KAND(NTMP2,NTMP).EQ.NTMP2)THEN
c                                      ISTA = ((MSTR-1)*L1INT)+MBITS
c                                        GO TO 4000
c                                   END IF
c1000                         CONTINUE
c                       END IF
