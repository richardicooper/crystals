...
C--SPREAD THE SYMMETRY OPERATORS
      CALL XSPRDD(N2*MD2,L2,KSV(1,12))
C--SPREAD THE INVERSE SYMMETRY OPERATORS
      CALL XSPRDD(N2I*MD2I,L2I,KSV(1,13))
C--SPREAD THE NON-PRIMITIVE LATTIVE TRANSLATIONS
      CALL XSPRDD(MD2P*N2P,L2P,KSV(1,14))
...
C--READ DOWN THE DATA FOR THE OLD LIST OF THIS TYPE
      LN=IULN
      CALL XRLIND(LN,LSN,IADDL,IOLEN,IOW,NOS,STORE(NFL))
...
1950  CONTINUE
      IF (ISSPRT .EQ. 0) THEN
      WRITE(NCWU,1850)J,ISTORE(J),ISTORE(J+1),(OVP(M,L),M=1,NWKO)
      ENDIF
      GOTO 1900
C--A LAYER SCALE OR AN ELEMENT SCALE TO BE PRINTED
2000  CONTINUE
      IF (ISSPRT .EQ. 0) THEN
      WRITE(NCWU,1850)J,ISTORE(J),ISTORE(J+1),(SCALE(M,KA),M=1,NWSC)
      ENDIF
      GOTO 1900
