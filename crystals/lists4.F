...
C----- LOAD THE MATRIX FOR THE GUI
      CALP=COS(STORE(L1P1+3))
      CBET=COS(STORE(L1P1+4))
      CGAM=COS(STORE(L1P1+5))
      RN=SQRT(1.0+2.0*CALP*CBET*CGAM-CALP**2-CBET**2-CGAM**2)
      D(1)=SIN(STORE(L1P1+3))/(RN*STORE(L1P1))
      D(2)=SIN(STORE(L1P1+4))/(RN*STORE(L1P1+1))
      D(3)=SIN(STORE(L1P1+5))/(RN*STORE(L1P1+2))
CCC      DO 100 I = 1,9
CCC            GUMTRX(I) = STORE(L1O1+I-1)
CCC            GUMTRX(I+9) = STORE(L1O2+I-1)
CCC            GUMTRX(I+18)= GUMTRX(I) * D1
CCC100   CONTINUE
...
C--COMPUTE THE REAL PART
      STORE(M3TR)=STORE(JU)*EXP(-STORE(JU+1)*SST)+STORE(JU+2)
     2 *EXP(-STORE(JU+3)*SST)+STORE(JU+4)*EXP(-STORE(JU+5)*SST)
     3 +STORE(JU+6)*EXP(-STORE(JU+7)*SST)+STORE(JU+8)+STORE(M3+1)
...
C--SET THE TYPE OF EACH SHIFT TO NORMAL
      N=MD33ST*N33ST
      M=-1
...
1000  CONTINUE
C--ASSIGN THE DEFAULT SHIFT VALUE OF UNITY
      N=MD33SV*N33SV
      A=1.
...
2550  CONTINUE
      CALL XMOVEI (JRQ(1,2), IRQ(1,2), NWRQ)
C----- SET INVERSION TO CHOLESKI OR EIGENVECTOR
      IRQ(3,5) = INVERT(2+ ISTORE(M33CD+8))
      KG=1
...
2650  CONTINUE
      ILEN=NWEND+1
      CALL XMOVEI (IRCALC(1,KG),IRQ(1,3),NWRQ)
      GOTO 2950
...
C
      DIMENSION IDEV(4)
      CHARACTER *80 CBUFF1, CBUFF2, CBUFF3, CLINE, CINPBF, COUTBF
      CHARACTER CEND*3
C
C----- TO HOLD REAL FORMAT EXPRESSION
      CHARACTER *8 CFMTXP
...
1000  CONTINUE
      ...
      ENDIF
C
      IF ( IDIRNM .EQ. 1 .OR. IDIRNM .EQ. 2 .OR. IDIRNM .EQ. 3) THEN
C----- FORMULA, SCATTERING OR PROPERTIES CARD
C -- SEARCH FOR THE PARAMETER KEYWORD
        IDWZAP = 0
        IF ( KFNDNP(IDWZAP) .LE. 0 ) GO TO 9810
C----- FIND NEXT AND LAST NON- BLANK ITEMS
...
C----- CHECK IF THERE IS ANY DATA ON CARD
      IF (ISTART .GT. ISIZE1) GOTO 9860
C
C----- LOOP BACK HERE FOR NEXT ARGUMENT
5000  CONTINUE
      IF(KCCARG (CBUFF1(1:ISIZE1), ISTART,1, IPOS1, IPOS2) .LE. 0)THEN
        IEND = +1
        IPOS2 = ISIZE1
      END IF
C----- TRY TO PROCESS AS NUMBER
      IF (KCCNUM(CBUFF1(IPOS1:IPOS2), IRESLT, ITYPE) .LE. 0) THEN
C----- NOT A NUMBER
        JARG = -1
      ELSE
C      A NUMBER
...
C      STORE CHARACTER
        WRITE(CTMP, 20) CBUFF1(IPOS1:IPOS2)
...
C-----  SPOT THE WAVELENGTH
        IF (ABS(VALUE-WAVE) .LE. .01 ) LAM = I
5350  CONTINUE
      IF (LAM .EQ. 0) GOTO 9700
...
9700  CONTINUE
      WRITE(NCAWU,9701)
      IF (ISSPRT .EQ. 0) WRITE ( NCWU, 9701)
      WRITE ( CMON, 9701)
      CALL XPRVDU(NCVDU, 1,0)
9701  FORMAT(' System files contain no entries for required wavelength')
      GOTO 9900
9710  CONTINUE
