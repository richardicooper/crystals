...
subroutine calcleverages(rindex, leverages, info)
...
do k=1, size(restraints_derivatives)
    if(rindex==restraints_derivatives(k)%irestraint) then
        associate(r => restraints_derivatives(k))
            ...
            listname=parameters_list(list(:))%name
            listlabel=parameters_list(list(:))%label
            listserial=parameters_list(list(:))%serial
