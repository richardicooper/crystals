...
type block_t !< blocks used during refinement
    real, dimension(:), allocatable :: designline !< line of the design matrix in the current block
end type
...
contains
    procedure, pass(self) :: printcontent !< print the content of the object (used for debugging)
end type
    ...
    do i=1, size(self%subrestraints)
        ...
        if(allocated(self%subrestraints(i)%blocks)) then
            ...
            do j=1, ubound(self%subrestraints(i)%blocks, 1)
                write(buffer, '("(7X,''('',I0,'')'',",I0,"(1X,1PE9.2))")') size(self%subrestraints(i)%blocks(j)%designline)
                write(*, buffer) size(self%subrestraints(i)%blocks(j)%designline), self%subrestraints(i)%blocks(j)%designline
            end do
        end if
    end do
end if
