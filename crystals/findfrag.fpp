...
C
C     COMMON /XDATC / NFL,LFL,NUL
C     COMMON/XDATA/STORE(131072)
...
C
C      DIMENSION ISTORE(131072)
C      DIMENSION ISTORE(2097152)
C
C     EQUIVALENCE (STORE(1),ISTORE(1))
C
C------------------------------------------------------
...
C      INCLUDE 'FIND.INC'
     ...
     2 L5LSC,M5LSC,MD5LSC,N5LSC, L5O,M5O,MD5O,N5O,
     3 L5LS,M5LS,MD5LS,N5LS, L5ES,M5ES,MD5ES,N5ES,
     4 L5BS,M5BS,MD5BS,N5BS, L5CL,M5CL,MD5CL,N5CL,
     5 L5PR,M5PR,MD5PR,N5PR, L5EX,M5EX,MD5EX,N5EX
C
...
C USE LON TO SWITCH ON/OFF
      ...
      ELSE IF (IMODE .EQ. 3) THEN
        ...
        CINFO = 'CURRENT '
      ELSE
        WRITE (6, *) 'INCORRECT IMODE VALUE IN CALL TO OUTSET'
        GO TO 9900
      END IF
      WRITE (NSCREE, *) '-------------------------------------------'
      WRITE (NSCREE, '(1X, A8, A12,A)') CINFO, ' INFO FROM: ', CLOCAT
      WRITE(NSCREE,'(1X,A33)')' LSETS  NSETS  LNEW  LPSL  NFL '
      WRITE(NSCREE,'(5(1X,I6))') LSETS, NSETS, LNEW, LPSL, NFL
...
C LOOP OVER ALL THE STRUCTURES
     ...
     1               1+((MSTR - LSTRUC)/MDSTRU)
          WRITE (NSCREE, '('' ['',I4,'']'',$)') ISTORE (MSETS +MOFF)
          NNO = ISTORE (MSTR)
          NSE = ISTORE (MSTR+15)
          WRITE (NSCREE, '(A1,$)') '{'
            DO 498, MNO = 1, NNO
            IF (LPRESE (MNO, MSETS, MOFF)) THEN
              WRITE (NSCREE, '(1X,I4, $)')  MNO
            END IF
498       CONTINUE
          WRITE (NSCREE,'(A1,$)') '}'
          MOFF = MOFF + NSE
...
C DO THE 'OR'
      ...
      WRITE (6, *)(ISTORE (MOPTI),MOPTI=LOPTI,LOPTI+NCPCKT-1)
      WRITE (6, *) ' '
      RETURN
      END
CODE FOR XADDST
...
C
      IF (NERRFI .EQ. 0) THEN
          WRITE (NSCREE, '(1X, A8)') 'NO ERROR'
      ELSE IF (NERRFI .LE. NEVAL) THEN
          WRITE (NSCREE, '(1X, A79)') '#FIND: ERROR '// CERRVA(NERRFI)
      ELSE
          WRITE (NSCREE, '(1X, A13, I3)') '#FIND: ERROR ', NERRFI
      END IF
      RETURN
      END
CODE FOR XFIND
...
C            WRITE (6, *) ' SERIAL NUMBER ', ISTORE(LFL+2), ' CT ', CT
            IPOSRE = NC + 3
...
\XUNITS
     ...
     2 L1M1,M1M1,MD1M1,N1M1,L1M2,M1M2,MD1M2,N1M2,
     3 L1O1,M1O1,MD1O1,N1O1,L1O2,M1O2,MD1O2,N1O2,
     4 L1S,M1S,MD1S,N1S,L1A,M1A,MD1A,N1A,L1C,M1C,MD1C,N1C
      COMMON/XLST02/IC,IL,G2,T2,L2C,M2C,MD2C,N2C,L2,M2,MD2,N2,
     2 L2I,M2I,MD2I,N2I,L2T,M2T,MD2T,N2T,L2P,M2P,MD2P,N2P,
     3 L2SG,M2SG,MD2SG,N2SG,L2CC,M2CC,MD2CC,N2CC
      COMMON/XLST05/L5,M5,MD5,N5, L5A,M5A,MD5A,N5A,
     2 L5LSC,M5LSC,MD5LSC,N5LSC, L5O,M5O,MD5O,N5O,
     3 L5LS,M5LS,MD5LS,N5LS, L5ES,M5ES,MD5ES,N5ES,
     4 L5BS,M5BS,MD5BS,N5BS, L5CL,M5CL,MD5CL,N5CL,
     ...
     2 L12B,M12B,MD12B,N12B, L12O,M12O,MD12O,N12O,
     3 L12LS,M12LS,MD12LS,N12LS, L12ES,M12ES,MD12ES,N12ES,
     4 L12BS,M12BS,MD12BS,N12BS, L12CL,M12CL,MD12CL,N12CL,
     5 L12PR,M12PR,MD12PR,N12PR, L12EX,M12EX,MD12EX,N12EX
      COMMON /XERVAL/ IERNOP,IERSFL,IERWRN,IERERR,IERSEV,IERCAT,
     2 IERPRG,IERABO,IERUNW
      COMMON /XOPVAL/ IOPCRY , IOPPUR , IOPLPC , IOPSSM , IOPMER ,
     2 IOPABS , IOPCOR , IOPREF , IOPFOU , IOPWEI , IOPANA , IOPTFC ,
...
C
               N = (MSTRUC-LSTRUC)/MDSTRU
               WRITE (NSCREE,'(A,I4)') 'WRITING OUTPUT FOR QUERY ',N
C
...
8000           CONTINUE
           ELSE
               WRITE (NSCREE,'(A,I4,A)') 'NO INSTANCES OF QUERY ',N,
     1                                  ' LOCATED'
           END IF
9000  CONTINUE
9900  CONTINUE
...
C IS FOUND
      ...
      IF ((NOCC .EQ. 1) .AND. LEXIST) WRITE (NSCREE, '(1X,A,A)')
