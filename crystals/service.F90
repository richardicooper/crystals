...
1200  FORMAT(' User error ', / 'Check your', &
       ' input data and your job control cards and parameters')
      CALL OUTCOL(1)
      GOTO 1350
...
!C----- CHECK IF WE ARE OUTPUTTING NON FATAL MESSAGES
      IF (ISSMSG .EQ. 0) THEN
          ...
          8000 , 1070 , 8000 , 8000 , 9910 ) , MSGNUM
      ELSE
      GO TO ( 1010 , 1020 , 1030 , 1040 , 1050 , &
       1060 , 1070 , 1080 , 1090 , 9910 ) , MSGNUM
      ENDIF
      GO TO 9910
!C
1010  CONTINUE
...
!Cdjwfeb2011
      ...
      integer icont, idump, lennm, istat, iscpnm, is, i, idscp
      integer, external :: kscsdc, kscidn, kdump
!C
...
!C-----  RECOVER SCRIPT NAME
        ...
        ' abandoned at line ',IRDREC(IFLIND)
...
9925  FORMAT ( 1X , 'An illegal code has been found in ' , &
       'the error routine -- Programming error' )
      CALL XFINAL ( 2 )
...
!C
!CODE FOR XERHND
      SUBROUTINE XERHNDnew(ICODE, ierflg)
!C -- GENERAL ERROR HANDLING ROUTINE FOR CRYSTALS
!C --
!C
!C
...
!C
      !INCLUDE 'XUNITS.INC'
      use xunits_mod, only: NCWU, NCVDU, NCEROR, NCAWU, IUSFLG
      !INCLUDE 'XSSVAL.INC'
      use xssval_mod
      !INCLUDE 'XERVAL.INC'
      use xerval_mod, only:
...
!C
      ...
      WRITE ( CMON,1005)   IA , IB , &
                              RESULT(ICODE), NWARN, WARNPL, NERR, ERRPL
      CALL XPRVDU(NCVDU, 3,0)
1005  FORMAT ( // , 1X , 2A4 , ' : Job ends ' , A8 , ' with ' , &
       I5 , ' warning' , A1 , ' and ' , I5 , ' error' , A1 )
!C
...
!C
      ...
      IF (ISSPRT .EQ. 0) THEN
        WRITE(NCWU,1000)
1000  FORMAT(//, &
       '   CCCCCCCC   RRRRRRRRRR  YY      YY   SSSSSSSS   TTTTTTTTTT', &
       '   AAAAAAAA   LL           SSSSSSSS ',/, &
       '  CCCCCCCCCC  RRRRRRRRRR   YY    YY   SSSSSSSSSS  TTTTTTTTTT', &
       '  AAAAAAAAAA  LL          SSSSSSSSSS',/, &
       '  CC          RR      RR    YY  YY    SS              TT    ', &
       '  AA      AA  LL          SS        ',/, &
       '  CC          RR      RR     YYYY     SS              TT    ', &
       '  AA      AA  LL          SS        ')
        WRITE(NCWU,1050)
1050  FORMAT( &
       '  CC          RR      RR      YY      SS              TT    ', &
       '  AA      AA  LL          SS        ',/, &
       '  CC          RRRRRRRRRR      YY      SS              TT    ', &
       '  AA      AA  LL          SS        ',/, &
       '  CC          RRRRRRRRRR      YY      SSSSSSSSSS      TT    ', &
       '  AA      AA  LL          SSSSSSSSSS',/, &
       '  CC          RRRR            YY      SSSSSSSSSS      TT    ', &
       '  AAAAAAAAAA  LL          SSSSSSSSSS')
        WRITE(NCWU,1100)
1100  FORMAT( &
       '  CC          RR RR           YY              SS      TT    ', &
       ...
       '  AA      AA  LL                  SS')
        WRITE(NCWU,1150)
1150  FORMAT( &
       '  CCCCCCCCCC  RR     RR       YY      SSSSSSSSSS      TT    ', &
       '  AA      AA  LLLLLLLLLL  SSSSSSSSSS',/, &
       '   CCCCCCCC   RR      RR      YY       SSSSSSSS       TT    ', &
       '  AA      AA  LLLLLLLLLL   SSSSSSSS ')
!C
      ENDIF
      ENDIF
!C
...
2110  FORMAT( &
       9X , 'To get help, type ' , A1 , 'HELP HELP', / &
       9X , 'To end, type      ' , A1 , 'FINISH' , / )
!C
!C
      CALL OUTCOL(1)
      IF ( ISSTML .EQ. 3) THEN
...
1150  FORMAT(1X,2A4,' : Processor time ',I4,' min',I4, &
       ' sec : Elapsed time ',I4,' min',I4,' sec')
      RETURN
      END
!C
!CODE FOR XTIME
...
!C----- RETURN IF A PHYSICAL DEVICE
      IF ((NEWFIL(1:LSSLPT) .EQ. CSSLPT(1:LSSLPT)) .OR. &
       (NEWFIL(1:LSSVDU) .EQ. CSSVDU(1:LSSVDU))) RETURN
!C
      IF (NEWFIL .EQ. ' ') THEN
...
!CS***
            ...
            NEWFIL = FLNAME( MYINDX )(1:LFNAME( MYINDX ) )//FLTYPE( MYINDX )
!CE***
#else
...
!CGUI{
      ...
      DATA GUIFUN(1) /'^^CO SET TEXTOUTPUT TEXTBOLD=NO TEXTUNDERLINE=NO TEXTITALIC=NO'/
...
!C
      ...
      character :: IHASH(12),MODE(3),MERROR(12)
!C
      !INCLUDE 'QSTORE.INC'
!C
!C
...
!C     START UP
      ...
      DATA IHASH(7)/'U'/,IHASH(8)/'P'/,IHASH(9)/' '/
      DATA IHASH(10)/' '/,IHASH(11)/' '/,IHASH(12)/' '/
!CDJWFEB10
!C     INCREASE I AND JHASH BUFFER TO 12 CHARACTERS
!C     ERROR
...
!C----- GET CPU AND ELLAPSED START TIME
      ...
      ELPTIM=FLOAT(I)
!C----- SET MODE
      JMODE=transfer(MODE(IQUN), JMODE)
...
!C -- IF THE END OF THE DAY HAS GONE BY, ADD 1440 MINUTES TO TIME
      ...
      WRITE(NCSU,2600)JHASH,JMODE,NCALL, &
       DELCPU,DELELP,TOTCPU
2600  FORMAT(1X,'INSTRUCTION ', 12A1, 1X, A1, 1X, I6, 1X, 3F10.2)
!C----- UPDATE VALUES
      OLDCPU=CPUTIM
      OLDELP=ELPTIM
...
4100  FORMAT(1X,'ERROR during ',12A1,' instruction')
      CALL XMOVE(MERROR,JHASH,12)
      GOTO 9000
!C
!C
!C----- FINAL CALL
