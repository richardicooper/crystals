...
C
C  JA      THE TYPE OF /FO/ INTERVAL :
C
C          0  BASED ON SQRT(/FO/)
C          1  BASED ON /FO/
C
...
C
     ...
     2 IPAR,IGEN,ICLS,JA,I,J,K,L,M,N,NX,NY,NZ,K1,K2,K3,NC,N1
\XUNITS
\XSSVAL
\XLISTI
\XCONST
...
C
#HOL      DATA PN(1)/'  SC'/,PN(2)/'ALE '/,PN(3)/'    '/,PN(4)/'    '/
#HOL      DATA PN(5)/'    '/
#HOLC--HEADING FOR THE NORMAL PLOT TYPE ENTRIES
#HOL      DATA PM(1)/'LOG('/,PM(2)/'<W*D'/,PM(3)/'ELTA'/,PM(4)/'**2>'/
#HOL      DATA PM(5)/')   '/
&HOL      DATA PN(1)/4H  SC/,PN(2)/4HALE /,PN(3)/4H    /,PN(4)/4H    /
&HOL      DATA PN(5)/4H    /
&HOLC--HEADING FOR THE NORMAL PLOT TYPE ENTRIES
&HOL      DATA PM(1)/4HLOG(/,PM(2)/4H<W*D/,PM(3)/4HELTA/,PM(4)/4H**2>/
&HOL      DATA PM(5)/4H)   /
...
C--CLEAR THE CLOCK CONTROL AND FETCH THE INPUT PARAMETERS
      CALL XTIME1(2)
      IF (  KRDDPV ( ISTORE(NFL) , 8 )  .LT.  0  ) GO TO 9930
...
C--BASED ON THE SQUARE ROOT OF /FO/
1250  CONTINUE
      VV=1.
      GOTO 1350
C--BASED ON /FO/
...
1950  FORMAT(///2X,A1,' axis')
      IF (ISSPRT .EQ. 0) THEN
      WRITE(NCWU,1000)(NOPE(I,4),I=1,4),PN
      ENDIF
...
2700  CONTINUE
      IF (ISSPRT .EQ. 0) THEN
      WRITE(NCWU,1000)(NOPE(I,3),I=1,4),PN
      ENDIF
      L=IGEN+IBASE(NR)
...
3850  FORMAT(///' Agreement analysis on ', A1,' index')
      IF (ISSPRT .EQ. 0) THEN
      WRITE(NCWU,1000)(NOPE(J,3),J=1,4),PM
      ENDIF
...
4300  FORMAT(///' Agreement analysis on reflection classes')
      WRITE(NCWU,1000)(NOPE(J,30),J=1,4),PM
      ENDIF
      L=ICLS
...
4600  FORMAT(///,' Agreement analysis on parity groups')
      IF (ISSPRT .EQ. 0) THEN
      WRITE(NCWU,1000)(NOPE(J,4),J=1,4),PM
      ENDIF
      L=IPAR
...
4870  CONTINUE
      IF (ISSPRT .EQ. 0) THEN
      WRITE(NCWU,4900)
      WRITE(NCWU,1000)(NOPE(J,1),J=1,4),PM
      ENDIF
      WRITE ( NCAWU , 4900 )
...
4920  FORMAT(1H ,F7.0,I6,2F9.0,2E10.2,2F5.0,3X,13A1)
...
C--PASS OVER EACH GROUP
      DO 5300 I=1,10000
C--COMPUTE THE CURRENT VALUE OF /FO/ TO BE PRINTED
      ACI=AC
      IF(JA)4950,4950,5000
4950  CONTINUE
      ACI=AC*AC
C--CHECK IF WE ARE AT THE END OF THE /FO/ VALUES
5000  CONTINUE
...
C--AGREEMENT ANALYSIS ON SIN(THETA)/LAMBDA RANGES
      ...
      WRITE(NCWU,5450)
      ENDIF
5450  FORMAT(///,' Agreement analysis on sin(theta)/lambda')
      IF (ISSPRT .EQ. 0) THEN
      WRITE(NCWU,1000)(NOPE(J,2),J=1,4),PM
      ENDIF
      AC=0.
      L=IGEN+NW
      NZ=0
