...
9925  FORMAT ( 1X , 'Illegal diffraction geometry flag ' , I5 )
      ...
      END
CODE FOR XSQRT
      SUBROUTINE XSQRT(FSQ, FSIGN, FABS)
C----- RETURN THE SIGNED AND UNSIGNED SQUARE ROOT OF A STRUCTURE AMPLITU
...
3550  CONTINUE
      CALL XSQRT(STORE(JFO), FSIGN, FABS)
      IF (FABS .LE. ZERO) THEN
            STORE(M6+12) = ZERO
      ELSE
            STORE(M6+12)=STORE(M6+12)/(2.*FABS)
      ENDIF
      STORE(JFO) = FSIGN
C--STORE THE REFLECTION
3600  CONTINUE
