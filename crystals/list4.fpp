...
C--CLEAR THE CLOCK CONTROL AND FETCH THE INPUT PARAMETERS
      CALL XTIME1(2)
      IF (  KRDDPV ( ISTORE(NFL) , 9 )  .LT.  0  ) GO TO 9930
...
5450  FORMAT(///,' Agreement analysis on sin(theta)/lambda')
      ...
      ENDIF
      IF (MONIT .NE. 1) THEN
      WRITE(CMON,'(A)')' Agreement analysis on sin(theta)/lambda'
      CALL XPRVDU(NCVDU,1,0)
      WRITE(CMON,4910)
      CALL XPRVDU(NCVDU,1,0)
      ENDIF
...
5610  FORMAT(1H ,F7.4,I6,2F9.0,2E10.2,2F5.0,13A1)
      IF (MONIT .NE. 1) THEN
      WRITE(CMON,5610)ACI,ISTORE(L+1)
      CALL XPRVDU(NCVDU, 1,0)
      ENDIF
      GOTO 5700
C--PRINT THE TOTALS FOR THIS GROUP OF REFLECTIONS
...
C--- OUTPUT TO SCREEN
      IF (MONIT .NE. 1) THEN
      WRITE(CMON,5610)ACI,ISTORE(L+1), STORE(L+2), STORE(L+3),
     2 (STORE(K+2),K=L+2,M) , IOUT
      CALL XPRVDU(NCVDU, 1,0)
      ENDIF
      NZ=0
