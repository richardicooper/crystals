...
CODE FOR KSCETR2
      FUNCTION KSCETR2 ( IN )
C
C -- THIS ROUTINE IMPLEMENTS THE 'EXTRACT2' INSTRUCTION IN SCRIPTS
C
C      INPUT PARAMETER :-
C
C            IN          DUMMY
...
C -- READ NEXT RECORD
C
        ISTAT = KRDLIN ( NCEXTR2 , CINPBF , LENGTH )
        IF ( ISTAT .LT. 0 ) GO TO 8910
        IF ( ISTAT .EQ. 0 ) GO TO 8920
C
      ELSE IF ( ITYPE .EQ. 5 ) THEN
