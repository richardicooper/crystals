...
C -- THESE ARE THE COMMON BLOCK OFFSETS LESS 1
      IMDAT1 = 9
      IMDAT2 = 10
C -- NOTE - THE 'ATOM' DIRECTIVE REQUIRES A SPACE OF MD5 (=14) WORDS 
C---- NEXT FREE ADDRESS IS 25. 
      IMDATM = 11
cdjwapr99
...
C -- READ A SINGLE SET OF DIRECTIVES
      ...
      IF ( IDIRNM .LT. 0 ) GO TO 1200
      IF ( IDIRNM .EQ. 0 ) GO TO 18000
...
C-C-C-INTRODUCTION OF ADDITIONAL DIRECTIVE-ADDRESSES FOR "SPECIALS"
      GO TO (
     1  1400, 2000, 2450, 3550, 5000, 5000, 5000, 5000, 6050, 6550,
     2  6570, 7250, 7300, 8750, 9000, 10400, 12000, 1200, 15100,
     3  15200, 15300, 15400, 15500, 15510, 8000, 9010, 15600, 6520,
     4  6500, 7500, 7600, 7510, 7520, 7530, 3000, 15250, 1320, 
     5  19920 ) , IDIRNM
      GO TO 19920
C
1320  CONTINUE
...
C----- SEE LIPSON & COCHRAN, EQN 302.3
     ...
     1 - (STORE(NFL)*STORE(NFL)+STORE(NFL+1)*STORE(NFL+1)+
     2 STORE(NFL+2)*STORE(NFL+2))
...
C -- INDICATE TO THE USER WHICH ATOMS WILL BE 'KEPT'
      IF ( LSTLVL .GT. 0 ) THEN
        IF ( MKEEP .GT. 0 ) THEN
          ...
          IF (ISSPRT .EQ. 0)
     1    WRITE ( NCWU , 10405 ) NKEEP , STORE(IADDR) , STORE(IADDR+1)
          WRITE ( NCAWU , 10405 ) NKEEP , STORE(IADDR) , STORE(IADDR+1)
          WRITE ( CMON,   10405 ) NKEEP , STORE(IADDR) , STORE(IADDR+1)
          CALL XPRVDU(NCVDU, 1,0)
10405     FORMAT ( 1X , 'The ' , I5 , ' atoms up to ' ,
     2 A4 , F5.0 , ' will be KEPT' )
        ENDIF
      ENDIF
      GO TO 1200
C
...
C    SUPPORTED.
      IF (ISSPRT .EQ. 0) WRITE ( NCWU , 2455 )
      WRITE ( NCAWU , 2455 )
      WRITE ( CMON, 2455)
      CALL XPRVDU(NCVDU, 1,0)
2455  FORMAT ( 1X , 'The READ directive has been replaced by ATOM')
      CALL XERHND ( IERWRN )
      GO TO 1200
C
C
...
C -- CHECK THERE IS SOME DATA
      IF ( ME .LE. 0 ) GO TO 19520
1550  CONTINUE
C -- CHECK FOR MORE DATA. READ IT. CHECK NO PARAMETERS SPECIFIED.
      IF ( KOP(8)  .LT. 0 ) GO TO 1200
      IF ( KATOMU(LN) .LE. 0 ) GO TO 19500
      IF ( ISTORE(MQ+5) .NE. 0 ) GO TO 19530
C
...
C -- CHECK FOR DATA. READ ATOMS. CHECK ONLY ONE ATOM
      IF ( ME .LE. 0 ) GO TO 8770
      IF ( KATOMU ( LN )  .LT. 0 ) GO TO 19500
      IF ( N5A .NE. 1 ) GO TO 19600
...
C -- READ ATOMIC OR OVERALL PARAMETER. CHECK ONLY ONE PARAMETER
      IF ( KOP(8) .LT. 0 ) GO TO 1200
      IF ( KCORCH(LN) .GT. 0 ) GO TO 9250
      IF ( KATOMU(LN) .LE. 0 ) GO TO 19500
      IF ( ISTORE(MQ+5) .NE. 1 ) GO TO 19550
9250  CONTINUE
...
9500  CONTINUE
      JT=JT+2
      IF ( KFDNUM ( APD(1) )  .LT. 0 ) GO TO 19500
C--CHECK IF THERE ARE ANY ATOMS
9550  CONTINUE
C----- CHECK FOR ATOM TYPE IF MODE IS TYPE CHANGE
      IF (ISTCF .EQ. 1) THEN
        IF (ME .LE. 0) GOTO 19540
        IF (ISTORE(MF) .GT. 0) GOTO 19540
        ITYPE = ISTORE(MF+2)
        ME = ME - 1
        MF = MF + LK2
      END IF
      IF(N5A)9050,9050,9600
C--LOOP OVER EACH ATOM IN THE LIST
9600  CONTINUE
      DO 10350 JW=1,N5A
C--GENERATE THE MOVED PARAMETERS
      IF ( KATOMS ( MQ , M5A , ITEMP )  .LT.  0 ) GO TO 19500
...
C--BRANCH ON THE TYPE OF CONDITION
      GO TO ( 9700 , 9750 , 9800 , 9850 , 9900 , 9950 , 10000 , 10050 ,
     2 19920 ) , JT
      GO TO 19920
C--ALPHA-NUMERIC 'EQUAL'
9700  CONTINUE
      IF(KCOMP(1,STORE(JU),APD(1),1,1))10100,10100,10300
C--ALPHA-NUMERIC 'NOT EQUAL'
9750  CONTINUE
      IF(KCOMP(1,STORE(JU),APD(1),1,1))10300,10300,10100
C--NUMERIC 'EQUAL'
9800  CONTINUE
      IF(ABS(STORE(JU)-APD(1))-ZERO)10300,10100,10100
C--NUMERIC 'NOT EQUAL'
9850  CONTINUE
      IF(ABS(STORE(JU)-APD(1))-ZERO)10100,10300,10300
C--'LESS THAN'
9900  CONTINUE
      IF(STORE(JU)-APD(1))10300,10100,10100
C--'LESS THAN OR EQUAL'
9950  CONTINUE
      IF(STORE(JU)-APD(1))10300,10300,10100
C--'GREATER THAN'
10000 CONTINUE
      IF(STORE(JU)-APD(1))10100,10100,10300
C--'GREATER THAN OR EQUAL'
10050 CONTINUE
      IF(STORE(JU)-APD(1))10100,10300,10300
...
8200  CONTINUE
      ...
      N5A=N5
      L12A=0
      IF(KATOMF(STORE(MQ+2),M5A,N5A,MD5A,L12A))8300,8400,8300
...
C----- LOOK FOR NEW ATOM NAME - IT MUST NOT ALREADY EXIST
      II=KATOMH(MQ)
C--CHECK THAT AN ATOM HAS BEEN FOUND CORRECTLY
      IF(II)8100,8150,8450
C--CHECK FOR THIS ATOM IN LIST 5
8450  CONTINUE
      M5A=L5
      N5A=N5
      L12A=0
      IF(KATOMF(STORE(MQ+2),M5A,N5A,MD5A,L12A) .EQ. 0) GOTO 19630
C--ATOM NOT IN LIST 5
...
C -- IF 'KEEP' HAS BEEN USED, RESET NOW
      IF ( MKEEP .GT. 0 ) THEN
        IF (ISSPRT .EQ. 0) WRITE ( NCWU , 15505 )
        WRITE ( NCAWU , 15505 )
        WRITE ( CMON, 15505)
        CALL XPRVDU(NCVDU, 1,0)
15505   FORMAT ( 1X , 'Previous KEEP instruction(s) cancelled' )
        CALL XERHND ( IERWRN )
        MKEEP = -1
        IKEEP = L5
        NKEEP = 0
...
C -- CHECK FOR AN OVERALL PARAMETER
      ISCALE = KOVPCH ( I )
      IF ( ISCALE .LT. 0 ) GO TO 19500
      IF ( ISCALE .EQ. 0 ) GO TO 3950
      L5A = M5A
C -- OVERALL PARAMETER FOUND. A NUMBER MUST BE READ NOW
      IF ( ME .LE. 0 ) GO TO 19540
      IF ( KSYNUM ( Z )  .NE.  0  ) GO TO 19540
      ME = ME - 1
      MF = MF + LK2
      DO 3900 I = 1 , N5A
      STORE(M5A) = Z
...
4200  CONTINUE
      ...
      JV=ISTORE(JU+1)-1
C--CHECK THE TYPE
      IF(JV)4450,4450,4550
C--ALPHA-NUMERIC VALUE EXPECTED
4450  CONTINUE
      IF ( ISTORE(MF) .GE. 0 ) GO TO 19540
      CALL XMOVE(STORE(MF+2),APD(1),1)
      GOTO 4600
C--NUMBER EXPECTED
4550  CONTINUE
      IF ( KSYNUM ( APD(1) )  .NE. 0 ) GO TO 19540
...
C-C-C-SUMMARIZE THE ABSOLUTE VALUES OF 5 LAST U[IJ]
        SUMUIJ=ABS(STORE(M5A+8))+ABS(STORE(M5A+9))+ABS(STORE(M5A+10))
     2        +ABS(STORE(M5A+11))+ABS(STORE(M5A+12))
C-C-C-CHECK ATOM BEING NON-ANISO OR ANISO WITHOUT VALUES FOR U[IJ]
        IF ( (STORE(JT) .GE. UISO)
     2       .OR. ( (STORE(JT) .LT. UISO)
     3              .AND. (SUMUIJ .LT. 0.0005) ) ) THEN
...
C-C-C-CHECK ATOM NOT BEING SPHERE OR BEING SPHERE WITHOUT SIZE-VALUE
        IF ( (NINT(STORE(JT)) .NE. 2)
     2       .OR. ( (NINT(STORE(JT)) .EQ. 2)
     3              .AND. (STORE(M5A+8) .LT. 0.0005) ) ) THEN
...
C-C-C-OR BEING LINE/RING WITHOUT VALUES FOR SIZE, DECL., AZIM.
        IF ( (NINT(STORE(JT)) .NE. NINT(APD(1)))
     2       .OR. ( (NINT(STORE(JT)) .EQ. NINT(APD(1)))
     3              .AND. (SUMUIJ .LT. 0.0005) ) ) THEN
...
C -- ALLOCATE SPACE REQUIRED BY NEW ATOMS
      I = KSTALL ( N5A * MD5 )
      DO 7000 JU = 1 , N5A
       IF  (  KATOMS ( MQ , M5A , M5 )  .LT. 0 ) GO TO 19500
...
C---- MAKE COPY
       ...
       STORE(M5+1)=STORE(M5+1) +1
CDJW99[] THIS MUST BE WRONG, BUT LEAVE ALONE TO AVOID CONFUSION FOR OLD 
C        USERS
       STORE(M5+1)=STORE(M5+1+MD5) +1
C----- NEW ATOM AT STORE(M5) - COMPUTE OFF SETS
...
CDJW99]
C----- SET U[ISO] = MEDIAN
C        STORE(M5+ 3)       = STORE(IBASE+2)
C        STORE(M5+ 3 + MD5) = STORE(IBASE+2)
        STORE(M5+ 7)       = STORE(IBASE+2)
        STORE(M5+ 7 + MD5) = STORE(IBASE+2)
...
C        CALL XZEROF( STORE(M5+7 + MD5), 6)
        CALL XZEROF( STORE(M5+8),       5)
        CALL XZEROF( STORE(M5+8 + MD5), 5)
       ENDIF
C
C----- CONDITIONAL BECAUSE OF OLD (13 PARAM) LIST 5S
       IF (MD5 .GE. 14) STORE(M5+13) = 0.0
...
C--CHECK TO SEE IF THE ATOM IS ALREADY ANISO
      IF(ABS(STORE(M5A+3))-UISO)2350,2250,2250
C--NOT ALREADY ANISO
2250  CONTINUE
      JV=M5A+2
      JW=L1C
...
C -- SET UP WORK SPACE
      ...
      M5A=L5
C--CHECK TO SEE IF THE CARD IS BLANK  -  ALL THE ATOMS TO BE CHANGED
      IF ( ME .LE. 0 ) GO TO 12200
12050 CONTINUE
C -- CHECK FOR MORE ARGUMENTS
      IF ( KOP ( 8 ) .GE. 0 ) GO TO 12100
C -- RETURN WORKSPACE
      CALL XSTRLL ( LTEMP )
      GO TO 1200
...
C--CHECK TO SEE IF THE ATOM IS ALREADY ISO
      IF(ABS(STORE(M5A+3))-UISO)12250,12350,12350
C--NOT ALREADY ISO
12250  CONTINUE
...
C----- BRANCH ON FUNCTION
      GOTO ( 5590 , 5560 , 5590 , 5570 , 5590 , 5020, 19920 ),IDIRNM
      GO TO 19920
5560   CONTINUE
C----- SUBTRACT
      Z=-Z
...
C -- CHECK TYPE OF PARAMETER
      JU = ISTORE(MQ+6) + MQ
      JV = ISTORE(JU+1) - 1
      IF ( JV .LE. 0 ) GO TO 19560
5300  CONTINUE
C--CHECK IF THERE ARE ANY ATOMS TO PROCESS
      IF(N5A)5050,5050,5600
...
C -- APPLY SYMMETRY TO ATOMIC COORDINATES
      ...
      ENDIF
      JU = M5A + JV
      GO TO ( 5800 , 5800 , 5900 , 5900 , 5910, 19920 ) , IDIRNM
      GO TO 19920
C--'ADD' OR 'SUBTRACT'
5800  CONTINUE
      STORE(JU)=STORE(JU)+Z
...
C----- Z
      ...
      GOTO 5950
5950  CONTINUE
      CALL XMDMON ( M5A,MD5A,1,IPOSIT,IPARTP,
     2                              5,3,MONLVL,2,0,ISTORE(IMONBF) )
      IPOSIT = IPOSIT + 1
      ICHNG = ICHNG + 1
      M5A=M5A+MD5A
6000  CONTINUE
      IF (IDIRNM .EQ. 5) THEN
        WRITE(NCAWU, 6001) NPERT , SQRT(ZZ),
     1 PERTOT/FLOAT(NPERT), SQRT (PERTSQ/FLOAT(NPERT))
        IF (ISSPRT .EQ. 0 ) WRITE(NCWU, 6001) NPERT , SQRT(ZZ),
     1 PERTOT/FLOAT(NPERT), SQRT (PERTSQ/FLOAT(NPERT))
      WRITE ( CMON,  6001) NPERT , SQRT(ZZ),
     1 PERTOT/FLOAT(NPERT), SQRT (PERTSQ/FLOAT(NPERT))
      CALL XPRVDU(NCVDU, 3,0)
6001  FORMAT('Mean perturbation should be approximately zero'/
     +' No of perturbations=', I5,5X,' Requested perturbation=', F10.4,/
     1'   Mean perturbation=', F10.4,'       Rms perturbation=', F10.4)
      ENDIF
      GOTO 5050
C
C
...
C -- READ SHIFT VECTOR. THREE NUMBERS OPTIONALLY SEPARATED WITH ','
      DO 6210 JW = 1 , 3
      IF ( ME .LE. 0 ) GO TO 19540
      IF ( KSYNUM ( APD(JW) )  .NE.  0  ) GO TO 19540
      ME = ME - 1
      MF = MF + LK2
      J = KOP ( 8 )
6210  CONTINUE
...
C--GENERATE THE NEW COORDS. BY ROTATION
      ...
      BPD(2)=STORE(M5A+5)
      BPD(3)=STORE(M5A+6)
      CALL XMLTTM(APD,BPD,STORE(M5A+4),3,3,1)
C----- NOW DO THE U'S
C-----TRANSFORM THE ANISOTROPIC TEMPERATURE FACTORS
      IF(ABS(STORE(M5A+3))-UISO)6300, 6350,6350
6300  CONTINUE
      J=M5A+7
      CALL XEXANI(J, JTEMP)
...
C----- 'INSERT' - INSERT A VALUE FROM LIST 3 OR 29 INTO 'SPARE'
      ...
      IF (KLST(I) .LE. 0 ) GOTO 19500
      IF ( KMDINS (I) .LT. 0 ) GOTO 19500
      GOTO 1200
C
C
C
...
C
      IF (ISSPRT .EQ. 0) WRITE ( NCWU , 19505 )
      WRITE ( NCAWU , 19505 )
      WRITE ( CMON, 19505)
      CALL XPRVDU(NCVDU, 1,0)
19505 FORMAT ( 1X , 'The previous directive has been ignored' )
      CALL XERHND ( IERWRN )
      LEF = LEF + 1
      GO TO 1200
C
19510 CONTINUE
      IF (ISSPRT .EQ. 0) WRITE ( NCWU , 19515 )
      WRITE ( NCAWU , 19515 )
      WRITE ( CMON, 19515)
      CALL XPRVDU(NCVDU, 1,0)
19515 FORMAT ( 1X , 'The previous directive should have been followed',
     2 ' by END' )
      GO TO 19500
C
19520 CONTINUE
      IF (ISSPRT .EQ. 0) WRITE ( NCWU , 19525 )
      WRITE ( NCAWU , 19525 )
      WRITE ( CMON, 19525)
      CALL XPRVDU(NCVDU, 1,0)
19525 FORMAT ( 1X , 'No arguments found' )
      GO TO 19500
C
19530 CONTINUE
      IF (ISSPRT .EQ. 0) WRITE ( NCWU , 19535 )
      WRITE ( NCAWU , 19535 )
      WRITE ( CMON, 19535)
      CALL XPRVDU(NCVDU, 1,0)
19535 FORMAT ( 1X , 'Parameter specifications are illegal for this ' ,
     2 'instruction' )
      GO TO 19500
C
19540 CONTINUE
      IF (ISSPRT .EQ. 0) WRITE ( NCWU , 19545 )
      WRITE ( NCAWU , 19545 )
      WRITE ( CMON, 19545)
      CALL XPRVDU(NCVDU, 1,0)
19545 FORMAT ( 1X , 'Argument missing or of wrong type' )
      GO TO 19500
C
19550 CONTINUE
      IF (ISSPRT .EQ. 0) WRITE ( NCWU , 19555 )
      WRITE ( NCAWU , 19555 )
      WRITE ( CMON, 19555)
      CALL XPRVDU(NCVDU, 1,0)
19555 FORMAT ( 1X , 'Wrong number of parameters specified' )
      GO TO 19500
C
19560 CONTINUE
      IF (ISSPRT .EQ. 0) WRITE ( NCWU , 19565 )
      WRITE ( NCAWU , 19565 )
      WRITE ( CMON, 19565)
      CALL XPRVDU(NCVDU, 1,0)
19565 FORMAT ( 1X , 'Incorrect parameter type specified' )
      GO TO 19500
C
19570 CONTINUE
      IF (ISSPRT .EQ. 0) WRITE ( NCWU , 19575 )
      WRITE ( NCAWU , 19575 )
      WRITE ( CMON, 19575)
      CALL XPRVDU(NCVDU, 1,0)
19575 FORMAT ( 1X , 'Only one argument group is allowed with this ' ,
     2 'directive' )
      GO TO 19500
C
19600 CONTINUE
      IF (ISSPRT .EQ. 0) WRITE ( NCWU , 19605 )
      WRITE ( NCAWU , 19605 )
      WRITE ( CMON, 19605)
      CALL XPRVDU(NCVDU, 1,0)
19605 FORMAT ( 1X , 'Only one atom may be specified' )
      GO TO 19500
C
19620 CONTINUE
      IF (ISSPRT .EQ. 0) WRITE ( NCWU , 19625 )
      WRITE ( NCAWU , 19625 )
      WRITE ( CMON, 19625)
      CALL XPRVDU(NCVDU, 1,0)
19625 FORMAT( 1X, ' Atom not found in LIST 5')
      GOTO 19500
C
19630 CONTINUE
      IF (ISSPRT .EQ. 0) WRITE ( NCWU , 19635 )
      WRITE ( NCAWU , 19635 )
      WRITE ( CMON, 19635)
      CALL XPRVDU(NCVDU, 1,0)
19635 FORMAT( 1X, ' Atom already in LIST 5')
      GOTO 19500
C
C
19900 CONTINUE
...
C--READ THE REMAINING DATA
      IF (  KRDDPV ( ISTORE(NFL) , 3 )  .LT.  0  ) GO TO 9910
...
C--CHECK THE ATOM TYPE
      IF(ABS(STORE(I+3))-UISO)1100,1050,1050
C--'ISO' ATOMS
1050  CONTINUE
...
C
9900  CONTINUE
      IF (ISSPRT .EQ. 0) WRITE ( NCWU , 9905 ) LB
      WRITE ( NCAWU , 9905 ) LB
      WRITE ( CMON, 9905) LB
      CALL XPRVDU(NCVDU, 1,0)
9905  FORMAT ( 1X , 'No new list ' , I3 , ' will be created' )
      CALL XERHND ( IERROR )
      GO TO 9000
9910  CONTINUE
      IF (ISSPRT .EQ. 0) WRITE ( NCWU , 9915 ) LA
      WRITE ( NCAWU , 9915 ) LA
      WRITE ( CMON, 9915) LA
      CALL XPRVDU(NCVDU, 1,0)
9915  FORMAT ( 1X , 'List ' , I3 , ' has not been changed ' )
      GO TO 9900
9920  CONTINUE
      IF (ISSPRT .EQ. 0) WRITE ( NCWU , 9925 ) LA
      WRITE ( NCAWU , 9925 ) LA
      WRITE ( CMON, 9925) LA
      CALL XPRVDU(NCVDU, 1,0)
9925  FORMAT ( 1X , 'Errors during processing of list ' , I3 )
C -- THIS CONDITION IS AN ERROR
      IERROR = IERERR
      GO TO 9900
9930  CONTINUE
      IF (ISSPRT .EQ. 0) WRITE ( NCWU , 9935 ) LA
      WRITE ( NCAWU , 9935 ) LA
      WRITE ( CMON, 9935) LA
      CALL XPRVDU(NCVDU, 1,0)
9935  FORMAT ( 1X , 'Modification of list ' , I3 , ' abandoned' )
      GO TO 9900
C
      END
C
...
C
CODE FOR XMDMON
      SUBROUTINE XMDMON ( ISTART , ISIZE , NDATA , IPOSIT , IPARTP ,
     2 IFUNC , LEVEL , IMNLVL , IMNCNT , IFLAG , ICONTL )
C
...
C
      ...
      ENDIF
C
      GO TO ( 1110 ,1120 ,1125 ,1125 ,1125 ,1130, 1130, 1130,
     1  9910 ) , IPARTP
      GO TO 9910
C
1110  CONTINUE
...
C
      IF ( NDATA .LE. 0 ) GO TO 1600
C
      DO 1400 I = 1 , NDATA
C
        GO TO ( 1310 , 1320 , 1330 , 9910 ) , ICLASS
        GO TO 9910
C
1310    CONTINUE
        GO TO 1350
1320    CONTINUE
        IND = IPOSIT + I - 1
        CALL XMOVE ( OVP(1,IND) , A(ITEM+1) , 2 )
        GO TO 1350
1330    CONTINUE
C----- SPECIAL OVERALL PARAMS
        CALL XMOVE ( SCALE(1,IPARTP) , A(ITEM+1) , 2 )
...
9910  CONTINUE
      ...
      WRITE ( CMON, 9915)
      CALL XPRVDU(NCVDU, 1,0)
9915  FORMAT ( 1X , 'Illegal LEVEL value selected' )
      CALL XERHND ( IERPRG )
      RETURN
      END
C
...
C
        ...
        ENDIF
C
        WRITE ( CMON, 1105)    ICONTL(IMDNDA) , ACTTYP(1:LENGTH)
1105    FORMAT (' The list currently contains ' , I5 , A )
        CALL XPRVDU(NCVDU, 1,0)
...
C
        ...
        LENGTH = LENGTH + 3
C
        WRITE ( CMON,   1205 ) ACTTYP(1:LENGTH) , OPERAT(1:LOPER)
        CALL XPRVDU(NCVDU, 1,0)
        IF (ISSPRT .EQ. 0) WRITE ( NCWU , '(A)' )  CMON(1)
        WRITE ( NCAWU , '(A)' )  CMON(1)
C
...
C
C
      GO TO ( 8100 , 8200 , 8300 , 8400 , 8500 ,
     2 8600, 8700, 8600, 8700, 9910 ) , ICONTL(IMDLVL)
      GO TO 9910
C
8100  CONTINUE
C -- BRIEF PRINT
      IF (ISSPRT .EQ. 0)
     1  WRITE(NCWU  , 8105) ( A(I), NINT(A(I+1)), I= 1,NA,2)
        WRITE(NCAWU , 8105) ( A(I), NINT(A(I+1)), I= 1,NA,2)
        WRITE ( CMON, 8105) ( A(I), NINT(A(I+1)), I= 1,NA,2)
        CALL XPRVDU(NCVDU, 1, 0)
8105  FORMAT (1X, 7( A4, I4, 3X) )
      GO TO 9000
C
8200  CONTINUE
C -- MEDIUM PRINT
...
C -- LOW LEVEL PRINT FOR OVERALL PARAMETERS
      IF (ISSPRT .EQ. 0)
     1 WRITE ( NCWU , 8405 ) ( A(I) , I = 1 , NA )
      WRITE ( NCAWU , 8405 ) ( A(I) , I = 1 , NA )
      WRITE ( CMON,   8405 ) ( A(I) , I = 1 , NA )
8405  FORMAT ( 1X, 8 ( 2A4 , 1X ) )
      GO TO 9000
8500  CONTINUE
C -- MEDIUM/HIGH LEVEL PRINT FOR OVERALL PARAMETERS
      IF (ISSPRT .EQ. 0)
     1 WRITE ( NCWU , 8505 ) ( A(I) , I = 1 , NA )
      WRITE ( NCAWU , 8505 ) ( A(I) , I = 1 , NA )
      WRITE ( CMON, 8505 ) ( A(I) , I = 1 , NA )
      CALL XPRVDU(NCVDU, 1,0)
8505  FORMAT ( 1X , 4 ( 2A4 , F9.3 , 2X ) )
      GO TO 9000
8600  CONTINUE
C -- LOW LEVEL PRINT FOR LAYER/BATCH/ELEMENT SCALE FACTORS
      IF (ISSPRT .EQ. 0)
     1 WRITE ( NCWU , 8605 ) ( A(I) , I = 1 , NA )
      WRITE ( NCAWU , 8605 ) ( A(I) , I = 1 , NA )
      WRITE ( CMON, 8605 ) ( A(I) , I = 1 , NA )
      CALL XPRVDU(NCVDU, 1,0)
8605  FORMAT ( 1X , 5 ( 2A4 , F4.0 , 3X ) )
      GO TO 9000
8700  CONTINUE
C -- MEDIUM/HIGH LEVEL PRINT FOR LAYER/BATCH/ELEMENT SCALE FACTORS
      IF (ISSPRT .EQ. 0)
     1 WRITE ( NCWU , 8705 ) ( A(I) , I = 1 , NA )
      WRITE ( NCAWU , 8705 ) ( A(I) , I = 1 , NA )
      WRITE ( CMON, 8705 ) ( A(I) , I = 1 , NA )
      CALL XPRVDU(NCVDU, 1,0)
8705  FORMAT ( 1X , 4 ( 2A4 , F4.0  , F6.3 , 1X ) )
      GO TO 9000
C
9000  CONTINUE
      NA = 0
...
C -- READ ATOMS. CHECK NO PARAMETERS.
      LN = 0
      IF ( KATOMU ( LN )  .LE. 0 ) GO TO 9900
      IF ( ISTORE(MQ+5) .NE. 0 ) GO TO 9920
      ICHNG = ICHNG + 1
C
...
C -- APPLY SYMMETRY WHILE MOVING ATOM BACK TO CORRECT POSITION
      IF (  KATOMS ( MQ , ITEMP , IMOVTO )  .LT. 0  ) GO TO 9900
C
C -- MONITOR ATOM
...
19535 FORMAT ( 1X , 'Parameter specifications are illegal for this ' ,
     2 'instruction' )
      GO TO 9900
9930  CONTINUE
      IF (ISSPRT .EQ. 0) WRITE ( NCWU , 19585 )
      WRITE ( NCAWU , 19585 )
      WRITE ( CMON, 19585)
      CALL XPRVDU(NCVDU, 1,0)
19585 FORMAT ( 1X , 'An atom may not be KEPT more than once' )
      GO TO 9900
      END
C
C
...
C
9910  CONTINUE
      IF (ISSPRT .EQ. 0) WRITE ( NCWU , 9915 ) ICOMP
      WRITE ( NCAWU , 9915 ) ICOMP
      WRITE ( CMON, 9915) ICOMP
      CALL XPRVDU(NCVDU, 1,0)
9915  FORMAT ( 1X , 'Type ' , A4 , ' has been repeated' )
      CALL XERHND ( IERWRN )
      RETURN
      END
C
