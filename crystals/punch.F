...
CODE FOR XPCH5D
      SUBROUTINE XPCH5D
C----- OUTOUT ATOMS IN 'CHIME' XYZ FORMAT
CDJWFEB2000
C--
\ISTORE
      DIMENSION A(3)
...
C---- OPEN THE .XYZ FILE
      ...
      CALL XRDOPN(6, KDEV , 'PUBLISH.XYZ', 11)
      WRITE (NCFPU1,1000) N5
1000  FORMAT(I6)
C----- OUTPUT A TITLE, FIRST 76 CHARACTERS ONLY
      WRITE(NCFPU1,'(1X,19A4)') (KTITL(I),I=1,19)
      M5 = L5
      DO 100 I = 1, N5
C--COMPUTE THE ORTHOGONAL COORDINATES OF THE ATOM
      CALL XMLTTM(STORE(L1O1),STORE(M5+4),A,3,3,1)
      WRITE (NCFPU1,1010) ISTORE(M5), A
1010  FORMAT(1X,A4,3F10.4)
      M5 = M5 + MD5
100   CONTINUE
C Close the .XYZ FILE
        CALL XMOVEI(KEYFIL(1,23), KDEV, 4)
