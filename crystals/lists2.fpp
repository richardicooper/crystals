...
CDJWFEB97>
      IF (IULN .EQ. 5) THEN
        ...
        ISAFE3 = ICOMMN(3)
        ISAFE4 = LSN
        CALL XGDBUP('READ    ',ISAFE1, ISAFE2, ISAFE3, ISAFE4,.FALSE.,4)
      ENDIF
CDJWFEB97<
      RETURN
9900  CONTINUE
...
CDJWFEB97>
      IF ((IULN .EQ. 5) .AND.
     1 (KRCLI (LN, ISAFE4, ISAFE1, ISAFE2, ISAFE3, J, ID) .GE. 0))
     2 THEN
C----   ONLY ISAFE4 (LSN) REQUIRED - OTHERS DUMMIES
        ISAFE1 = ICOMMN(1)
        ISAFE2 = ICOMMN(4)
        ISAFE3 = ICOMMN(3)
        CALL XGDBUP('WRITE   ',ISAFE1, ISAFE2, ISAFE3, ISAFE4,.FALSE.,4)
      ENDIF
CDJWFEB97<
      RETURN
      END
...
C
CODE FOR KDECOR
      FUNCTION KDECOR(IADD,INCREM)
C--ALTER THE ADDRESS 'IADD' NEGATIVELY BY THE AMOUNT 'INCREM-1'
C
C  IADD    ADDRESS TO BE ALTERED
C  INCREM  NUMBER PLUS ONE TO BE SUBTRACTED FROM 'IADD'.
C
C--'LFL' IS UPDATE BY THIS PROCESS SO THAT IT IS EQUAL TO 'IADD-INCREM'.
C
C--RETURN VALUE IS THE NEW ADDRESS 'IADD-INCREM+1'.
C
C--'INCREM' ASSUMES A MINIMUM VALUE OF 2.
C
C--
C
C
...
C
CODE FOR XSTRLH
      SUBROUTINE XSTRLH(IADDR)
C -- MAKES LFL EQUAL TO HIGHER OF LFL AND IADDR
C --
\STORE
C --
C -- IF ADDRESS IS LESS THAN OR EQUAL TO ZERO THEN RETURN
      IF (IADDR.LE.0) RETURN
      IF (IADDR.GT.65536) RETURN
      LFL=MAX0(NFL,IADDR)
      RETURN
      END
C
