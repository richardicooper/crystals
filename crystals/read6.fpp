...
6600  CONTINUE
      IF(STORE(JFO) .GE. ZERO)THEN
C--    /FO/ **2 IS OKAY  -  TAKE THE SQUARE ROOT
        STORE(JFO)=SQRT(STORE(JFO))
      ELSE
        STORE(JFO) = SIGN(1.0, STORE(JFO)) *SQRT(ABS(STORE(JFO)))
      ENDIF
      IF(ABS(STORE(JFO)) .GE. ZERO)THEN
        STORE(M6+12)=STORE(M6+12)/(2.*ABS(STORE(JFO)))
      ELSE
C----- IF THE DATA HAVE NOT BEEN PRE-SCALED, THE INTENSITIES WILL BE
C      INTEGER VALUES, SO THAT THE CLOSEST OBSERVATION TO ZERO IS 1
        STORE(M6+12) = STORE(M6+12) / 2.
      ENDIF
