...
C--LIST TYPE 4  -  STORE THE WEIGHTING SCHEME TYPE
1050  CONTINUE
      I=ISTORE(JA+8)
      ISTORE(I)=ISTORE(JA)
      ISTORE(I+1)=ISTORE(JA+1)
C----- NOW STORE THE REAL VARIABLES
      I = ISTORE(JA+12)
      STORE(I) = FLOAT (ISTORE(JA+2))
      STORE(I+1) = FLOAT (ISTORE(JA+3))
      GOTO 2300
C
...
1400  CONTINUE
      CALL XMVSPD(IB,TITLE(1),20)
C--FORM THE TITLE INTO A4 CHARACTERS
      CALL XFA4CS ( LCMAGE(NC) , TITLE(1) , N )
...
C
      IF (ISSPRT .EQ. 0) THEN
      WRITE ( NCWU , 2915 ) TITLE
      ENDIF
      WRITE ( NCAWU , 2915 ) TITLE
      WRITE ( CMON, 2915 ) TITLE
      CALL XPRVDU(NCVDU, 1,0)
