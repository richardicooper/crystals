...
CODE FOR ZTXT
C      SUBROUTINE ZTXT ( IX , IY , CTEXT , IMESSG)
      SUBROUTINE ZTXT (  CTEXT )
...
C - CHECK FOR MOUSE CLICKS

      I = KMOUSE(CC)

      IF (I.EQ.0) THEN

C ADD ATOM NAME TO THE LINE

C ENSURE CURRENT LINE ENDS WITH A SPACE
          IF (ITC .GT. 0) THEN
...
CODE FOR ZPRMPT
      SUBROUTINE ZPRMPT (CTEXT)

C Output a line of text above the user's input line.
\CAMPAR
\CAMCOM
\CAMANA
\CAMDAT
...
\XIOBUF
      ...
      IXX = 10
      IYY = 2.0 * YCENS + 5
      CALL ZDTEXT(CTEXT,IXX,IYY,IBTEXT)
      RETURN
      END
...
CODE FOR KMSADD
      SUBROUTINE KMSADD(JX,JY,JF)
C ADD MOUSE INFO TO THE BUFFER
c
C INEVEN -       NUMBER OF MOUSE COORDS BUFFERED
C IREVEN -       ADDRESS OF NEXT EVENT TO READ IN ICORDS
C IWEVEN -       ADDRESS OF NEXT EVENT TO WRITE IN ICORDS
