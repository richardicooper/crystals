...
C
     ...
     2 MACROS(96000)
      COMMON/XCARDS/NCHAR,NCHARM,NED(80),ICARD(80),I,J,ID,JD
...
1550  FORMAT(/27H ILLEGAL MACRO PARAMETER : ,80A1)
      STOP 04
...
2750  CONTINUE
      WRITE(NCWU,2760)NED
2760  FORMAT(/28H ILLEGAL EDIT INSTRUCTION : ,80A1)
      STOP 01
2800  CONTINUE
...
3000  CONTINUE
      WRITE(NCWU,3050)NED
3050  FORMAT(/28H LINE NUMBER OUT OF ORDER : ,80A1)
      STOP 02
...
2200  FORMAT(/27H ILLEGAL MACRO PARAMETER : ,80A1)
      STOP 07
...
C -- CHECK FOR MACHINE DEPENDENT CODE
      CALL XCHMAC(IBUF(1))
      IF((.NOT.LSTRIP).OR.((IBUF(1).NE.ICOM).OR.(IBUF(2).NE.ICOM2)))
     1    WRITE(NCPU,900)(IBUF(ITEMP),ITEMP=1,NCHAR)
...
5500  CONTINUE
      IF((.NOT.LSTRIP).OR.((LCARD(1).NE.ICOM).OR.(LCARD(2).NE.ICOM2)))
     1 WRITE(NCPU,900)(LCARD(ITEMP),ITEMP=1,NCHAR)
6000  CONTINUE
      RETURN
      END
CODE FOR XCHMAC
