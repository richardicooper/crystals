...
def parseissue(journalissue):
    html = journalissue.read()
...
if wholeyear:
    ...
    while(True):
        i=i+1
        path="%s-%d-%02d"%(journal, year, i)
        journalissue = "https://journals.iucr.org/%s/issues/%d/%02d/00/index.html"%(journal, year, i)
        print(journalissue)
        try:
